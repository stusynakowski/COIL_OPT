...
from consistent_coil_utils import (
    RigidCurve,
    build_coils_with_consistency,
    make_rigid_coils,
    make_shared_shape_coils,
)
...
# Weight for soft shape consistency (shared shape via Lagrange-style penalty):
SHAPE_MATCH_WEIGHT = 1e2
...
# ---- Consistent coil shape configuration ----
# Indices of base coils whose shape should be frozen (rigid-body DOFs only).
# Disable rigid-wrappers while we resolve coil constructor type issues.
RIGID_COIL_INDICES = None

# Shared-shape groups: disable wrapper-based shared shapes for compatibility.
SHARED_SHAPE_GROUPS = None

# Weight for softly constraining the shared template to remain near its initial
# shape (acts like a Lagrange multiplier). Set to 0 while shared-shape wrappers
# are disabled.
RIGIDITY_PENALTY = 1e2
...
# Build standard coils (disable shared-shape wrappers for now).
...
# Soft template pin disabled while shared-shape wrappers are off.
...
def fun(dofs):
    ...
    # Manual shape-consistency penalty tying all coil DOFs to coil 0.
    J_shape = 0.0
    for i in range(1, ncoils):
        diff = base_curves[i].x - base_curves[0].x
        J_shape += 0.5 * SHAPE_MATCH_WEIGHT * np.dot(diff, diff)
        def _flatten_indices(dct):
            parts = []
            for v in dct.values():
                if isinstance(v, slice):
                    parts.append(np.arange(v.start, v.stop, v.step or 1, dtype=int))
                elif isinstance(v, tuple) and len(v) == 2:
                    parts.append(np.arange(v[0], v[1], dtype=int))
                else:
                    parts.append(np.atleast_1d(v).astype(int))
            return np.concatenate(parts)

        idx_i = _flatten_indices(base_curves[i].dof_indices)
        idx_0 = _flatten_indices(base_curves[0].dof_indices)
        grad[idx_i] += SHAPE_MATCH_WEIGHT * diff
        grad[idx_0] -= SHAPE_MATCH_WEIGHT * diff
    J += J_shape
    jf = Jmpi.J()
