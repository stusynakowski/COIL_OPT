...
def build_overlay_figure(stage, coil_width, dark_mode):
    """Build a figure with all coils centred + aligned on top of each other."""
    curves_file = OUT_DIR / f"curves_{stage}.vtu"
    coils = load_coils_from_vtu(str(curves_file))
    ...
    for c in coils:
        # Use equally‑spaced subset if lengths differ
        if len(c) != n_pts:
    ...
    # Use first coil as reference
    ref = centred[0]
    aligned = [ref]
    rmsds = [0.0]

    for i in range(1, len(centred)):
        al, rmsd = _best_circular_shift(ref, centred[i])
        aligned.append(al)
        rmsds.append(rmsd)

    fig = go.Figure()
    for i, pts in enumerate(aligned):
        xyz = np.concatenate([pts, pts[:1]], axis=0)
        fig.add_trace(
            go.Scatter3d(
                x=xyz[:, 0], y=xyz[:, 1], z=xyz[:, 2],
                mode="lines",
                line=dict(width=coil_width, color=COLORS[i % len(COLORS)]),
                name=f"Coil {i + 1}  (RMSD {rmsds[i]:.4f})",
            )
        )
    ...
    fig.update_layout(
        title=f"Coil Shape Overlay — {stage}  (aligned to Coil 1)",
        scene=dict(
...
if overlay_coils:
    ...
    overlay_fig, n_overlay, rmsds = build_overlay_figure(
        stage, coil_width, dark_mode
    )
    ...
    if rmsds:
        ...
        df = pd.DataFrame({
            ...
            "RMSD vs Coil 1": [f"{r:.6f}" for r in rmsds],
        })
